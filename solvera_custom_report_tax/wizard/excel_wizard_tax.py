    ...
    def export_excel(self):
        ...
        query = """
            SELECT DISTINCT ON (aml.id) aml.id as line_id,	am.invoice_date as dates, am.name as invoice, rp.name as partner_id,
                    ...
                    TO_CHAR(am.invoice_date_due, 'DD-MM-YYYY') as dates_due,pc.name as categ, rb.name as branch,rpp.internal_name as parent
                    ,am.branch_id,rpp.state_sale as sale, rpc.name as tags
            FROM account_move_line aml
                JOIN account_move am on aml.move_id =am.id
                LEFT JOIN res_partner rp on am.partner_id = rp.id
                LEFT JOIN res_partner rpp on rp.parent_id = rpp.id
				LEFT JOIN res_partner_res_partner_category_rel as rp_rpc on rp_rpc.partner_id = rpp.id
				LEFT JOIN res_partner_category as rpc on rpc.id = rp_rpc.category_id
        ...
        filename = 'Report Activiy Acounting2.xls'
        ...
        sheet.write('E1', 'Tags', normal_center_border)
        sheet.write('F1', 'SUMBER PENJUALAN', normal_center_border)
        sheet.write('G1', 'Nomor SO', normal_center_border)
        sheet.write('H1', 'Tanggal SO', normal_center_border)
        sheet.write('I1', 'Nomor DO', normal_center_border)
        sheet.write('J1', 'Tanggal DO', normal_center_border)
        sheet.write('K1', 'Nomor Invoice', normal_center_border)
        sheet.write('L1', 'Payment Reference', normal_center_border)
        sheet.write('M1', 'KODE BARANG', normal_center_border)
        sheet.write('N1', 'NAMA BARANG', normal_center_border)
        sheet.write('O1', 'UOM', normal_center_border)
        sheet.write('P1', 'QTY', normal_center_border)
        sheet.write('Q1', 'HARGA', normal_center_border)
        sheet.write('R1', 'DISC', normal_center_border)
        sheet.write('S1', 'TOTAL', normal_center_border)
        sheet.write('T1', 'Status AR', normal_center_border)
        sheet.write('U1', 'Tanggal Pelunasan', normal_center_border)
        sheet.write('V1', 'Salesperson', normal_center_border)
        sheet.write('W1', 'Sales Team', normal_center_border)
        sheet.write('X1', 'Due Date', normal_center_border)
        sheet.write('Y1', 'Category', normal_center_border)
        sheet.write('Z1', 'Branch', normal_center_border)
        ...
        for value in sorted(pick_ids, key=lambda x: x['invoice']):
                ...
                sheet.write('E'+str(rows+1), value[str('tags')], normal_left_border)
                sheet.write('F'+str(rows+1), state, normal_left_border)
                # sheet.write('E'+str(rows+1), value[str('tax_id')], normal_left_border)
                sheet.write('G'+str(rows+1), value[str('invoice_origin')], normal_left_border)
                sheet.write('H'+str(rows+1), value[('order_date')], date_format)
                sheet.write('I'+str(rows+1), value[str('picking')], number_center)
                sheet.write('J'+str(rows+1), value[('picking_date')], date_format)
                sheet.write('K'+str(rows+1), value[str('invoice')], number_center)
                sheet.write('L'+str(rows+1), value[str('refe')], number_center)
                # sheet.write('L'+str(rows+1), value[str('tax_id')], number_center)
                sheet.write('M'+str(rows+1), value[str('product_code')], number_center)
                sheet.write('N'+str(rows+1), value[str('product_id')], normal_left_border)
                sheet.write('O'+str(rows+1), value[str('uu')], normal_left_border)
                sheet.write('P'+str(rows+1), value[str('quantity')], normal_left_border)
                # sheet.write('N'+str(rows+1), value[str('volume')], normal_left_border)
                sheet.write('Q'+str(rows+1), value[str('price_unit')], normal_left_border)
                sheet.write('R'+str(rows+1), value[str('diskon')], normal_left_border)
                sheet.write('S'+str(rows+1), value[str('subtotal')], normal_left_border)
                sheet.write('T'+str(rows+1), value[str('payment_state')], normal_left_border)
                sheet.write('U'+str(rows+1), value[('date_create')], date_format)
                sheet.write('V'+str(rows+1), value[str('login')], normal_left_border)
                sheet.write('W'+str(rows+1), value[str('name')], normal_left_border)
                # sheet.write('Y'+str(rows+1), value[str('joko')], normal_left_border)
                sheet.write('X'+str(rows+1), value[('dates_due')], normal_left_border)
                sheet.write('Y'+str(rows+1), value[('categ')], normal_left_border)
                sheet.write('Z'+str(rows+1), value[('branch')], normal_left_border)
                state=False
        ...
        action = {
            ...
            'url': 'web/content/?model=excel.wizard&id='+str(self.id)+\
