...
class WizCommission(models.TransientModel):
    ...
    def export_sale(self):
        ...
        if self.env.user.branch_id.id == 1:
            query = """
               SELECT DISTINCT ON (aml.id) aml.id as line_id,	am.invoice_date as dates, am.name as invoice, rp.name as partner_id,
                    ...
                    TO_CHAR(am.invoice_date_due, 'DD-MM-YYYY') as dates_due,pc.name as categ, rb.name as branch,rpp.internal_name as parent ,am.branch_id,rpp.state_sale as sale
            ...
            WHERE am.move_type in ('out_refund','out_invoice') and aml.partner_id is not null  AND am.state != 'cancel' and aml.product_id is not null and aml.price_unit > 0 and aml.is_anglo_saxon_line is null and am.branch_id = 1 and am.invoice_date >= %s AND am.invoice_date <= %s 
                ORDER BY line_id
            """
            ...
            for value in sorted(pick_ids, key=lambda x: x['invoice']):
                    ...
                    sheet.write('Y'+str(rows+1), value[('branch')], normal_left_border)
                    state=False
            ...
            return action
        if self.env.user.branch_id.id == 2:
            query = """
                SELECT DISTINCT ON (aml.id) aml.id as line_id,	am.invoice_date as dates, am.name as invoice, rp.name as partner_id,
                        ptemp.name as product_id, ptemp.default_code as product_code ,
                        aml.price_unit, aml.discount, aml.quantity,am.state, am.payment_state,
                        ...
                        ,am.branch_id,rpp.state_sale as sale
                FROM account_move_line aml
                    JOIN account_move am on aml.move_id =am.id
                    LEFT JOIN res_partner rp on am.partner_id = rp.id
                    LEFT JOIN res_partner rpp on rp.parent_id = rpp.id
                    ...
                    LEFT JOIN account_payment ap ON am2.id = ap.move_id
                    LEFT JOIN sale_order so ON am.invoice_origin =so.name
                WHERE am.branch_id = 2 and am.move_type in ('out_refund','out_invoice') and aml.partner_id is not null  AND am.state != 'cancel' and aml.product_id is not null and aml.price_unit > 0 and aml.is_anglo_saxon_line is null and am.invoice_date >= %s AND am.invoice_date <= %s 
                ORDER BY line_id
            """
            params = (self.date_from, self.date_to,)
            self.env.cr.execute(query,params)
        ...
        if self.env.user.branch_id.id == 3:
            query = """
                SELECT DISTINCT ON (aml.id) aml.id as line_id,	am.invoice_date as dates, am.name as invoice, rp.name as partner_id,
                        ptemp.name as product_id, ptemp.default_code as product_code ,
                        aml.price_unit, aml.discount, aml.quantity,am.state, am.payment_state,
                ...
                FROM account_move_line aml
                    ...
                    LEFT JOIN account_payment ap ON am2.id = ap.move_id
                    LEFT JOIN sale_order so ON am.invoice_origin =so.name
                WHERE am.branch_id = 3 and am.move_type in ('out_refund','out_invoice') and aml.partner_id is not null  AND am.state != 'cancel' and aml.product_id is not null and aml.price_unit > 0 and aml.is_anglo_saxon_line is null and am.invoice_date >= %s AND am.invoice_date <= %s 
                ORDER BY line_id
            """
            params = (self.date_from, self.date_to,)
            self.env.cr.execute(query,params)
