...
class GeneralView(models.TransientModel):
    ...
    def _get_accounts(self, accounts, init_balance, display_account, data):
        ...
        print(WHERE,'test_where',where_params)
    ...
    def _get_accounts_custom(self, accounts, init_balance, display_account, data):
    ...
    def get_dynamic_xlsx_report(self, data, response ,report_data, dfr_data):
        ...
        for rec_data in report_data_main:
            ...
            # print(report_data_main,"test_print")
            # {'account_id': 1390, 'code': '120001008', 'id': 1390, 'name': 
            # 'BCA USD - 7999', 'debit': 147900000, 'credit': 191542589, 'balance': -43642589}
            # WHERE l.account_id IN %s test_where [1, 'line_section', 'line_note', 'cancel', 1, 3, 1, 2]

            for line_data in self.env['account.move.line'].search([('account_id','=',rec_data['account_id'])]):
                
            # for line_data in self.get_accounts_line(rec_data['account_id'],'').get('report_lines'):
                row += 1
                ...
                sheet.write(row + 1, col + 2, line_data.date)
                sheet.write(row + 1, col + 3, line_data.account_id.name, txt)
                sheet.write(row + 1, col + 4, line_data.move_id.partner_id.name, txt)
                sheet.write(row + 1, col + 5, line_data.move_id.name, txt)
                sheet.write(row + 1, col + 6, line_data.name, txt)
                sheet.write(row + 1, col + 7, line_data.debit, txt)
                sheet.write(row + 1, col + 8, line_data.credit, txt)
                sheet.write(row + 1, col + 9, line_data.balance, txt)
