...
class ProductSample(models.Model):
    _name = "product.sample"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Form Sample Product"
    _order = "id desc"

    name = fields.Char(
        'Sample Reference', default="Sample",
        readonly=True, required=True,
        states={'draft': [('readonly', False)]})

    sample_line_ids =  fields.One2many('product.sample.line','sample_id',string="Sample Line")
    property_stock_account = fields.Many2one(
        'account.account', 'Stock Sample Account',required=True, company_dependent=True,
    ...
    move_ids = fields.One2many(
        'stock.move', 'sample_id', string='Created Moves',
    ...
    def copy_data(self, default=None):
        ...
        return super(ProductSample, self).copy_data(default)
    ...
    def unlink(self):
        ...
        return super(ProductSample, self).unlink()
    ...
    def action_validate(self):
        ...
        inventory_lines = self.sample_line_ids.filtered(lambda l: l.product_id.tracking in ['lot', 'serial'] and not l.prod_lot_id and l.theoretical_qty != l.product_qty)
        lines = self.sample_line_ids.filtered(lambda l: float_compare(l.product_qty, 1, precision_rounding=l.product_uom_id.rounding) > 0 and l.product_id.tracking == 'serial' and l.prod_lot_id)
        ...
        self._action_done()
        self.sample_line_ids._check_company()
        self._check_company()
    ...
    def change_journal(self):
        for this in self:
            account_move = self.env['account.move'].search([
                        ('stock_move_id.id', 'in', this.move_ids.ids)
                    ])
            for categ in this.sample_line_ids:
                for line in account_move:
                    for lines in line.line_ids:
                        if categ.product_id == lines.product_id:
                            if categ.categ_id.property_stock_account_output_categ_id == lines.account_id:
                                lines.update({
                                    'account_id': self.property_stock_account.id,
                                })
    ...
    def update_stock_move(self):
        for line in self.move_ids:
            if line.location_id.branch_id.id:
                line.update({
                    'branch_id':line.location_id.branch_id.id
                })
            else:
    ...
    def _action_done(self):
        negative = next((line for line in self.mapped('sample_line_ids') if line.product_qty < 0 and line.product_qty != line.theoretical_qty), False)
        if negative:
    ...
    def action_check(self):
        ...
        for inventory in self.filtered(lambda x: x.state not in ('done','cancel')):
            ...
            inventory.sample_line_ids._generate_moves()

    def action_cancel_draft(self):
        self.mapped('move_ids')._action_cancel()
        self.sample_line_ids.unlink()
    ...
    def _action_start(self):
        ...
        for inventory in self:
            ...
            if not inventory.sample_line_ids and not inventory.start_empty:
                self.env['product.sample.line'].create(inventory._get_inventory_lines_values())
    ...
    def action_open_inventory_lines(self):
        ...
        action = {
            'type': 'ir.actions.act_window',
            'views': [(self.env.ref('sovlera_inventory_journal.product_sample_line_tree2').id, 'tree')],
            'view_mode': 'tree',
            'name': _('Inventory Lines'),
            'res_model': 'product.sample.line',
        }
        context = {
    ...
    def _get_inventory_lines_values(self):
        ...
        for product_data in self.env.cr.dictfetchall():
            ...
            product_data['sample_id'] = self.id
...
class ProductSampleLine(models.Model):
    _name = "product.sample.line"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Form Sample Product"
    _order = "id desc"
    ...
    sample_id = fields.Many2one(
        'product.sample', 'sample', check_company=True,
    ...
    product_qty = fields.Float(
        'Counted Quantity',
        digits='Product Unit of Measure', default=0,force_save=1)
    ...
    company_id = fields.Many2one(
        'res.company', 'Company', related='sample_id.company_id',
        index=True, readonly=True, store=True)
    state = fields.Selection('Status', related='sample_id.state')
    ...
    simple_diff_qty = fields.Float('Difference')

    @api.onchange('simple_diff_qty')
    def simple_dif(self):
        for i in self:
            total = i.simple_diff_qty + i.theoretical_qty
            i.product_qty = total
    ...
    def create(self, vals_list):
        ...
        res = super(ProductSampleLine, self).create(vals_list)
        res._check_no_duplicate_line()
        return res

    def write(self, vals):
        res = super(ProductSampleLine, self).write(vals)
    ...
    def _check_no_duplicate_line(self):
        domain = [
            ...
            '|', ('sample_id', 'in', self.sample_id.ids), ('sample_id', '=', None),
        ]
        groupby_fields = ['product_id', 'location_id', 'partner_id', 'package_id', 'prod_lot_id', 'sample_id']
        lines_count = {}
        ...
        for line in self:
            key = (line.product_id.id, line.location_id.id, line.partner_id.id, line.package_id.id, line.prod_lot_id.id, line.sample_id.id)
    ...
    def _get_move_values(self, qty, location_id, location_dest_id, out):
        self.ensure_one()
        return {
            'name': _('INV:') + (self.sample_id.name or ''),
            'product_id': self.product_id.id,
            'product_uom': self.product_uom_id.id,
            'product_uom_qty': qty,
            'date': self.sample_id.date,
            'company_id': self.sample_id.company_id.id,
    ...
    def _get_virtual_location(self):
        return self.product_id.with_context(with_company=self.company_id.id).property_stock_inventory
    ...
    def _refresh_inventory(self):
        return self[0].sample_id.action_open_inventory_lines()
    ...
    def action_reset_product_qty(self):
        ...
        impacted_lines = self.env['product.sample.line']
        for line in self:
            if line.state == 'done':
                continue
            impacted_lines |= line
    ...
    def _search_difference_qty(self, operator, value):
        ...
        if not self.env.context.get('default_sample_id'):
            raise NotImplementedError(_('Unsupported search on %s outside of an Inventory Adjustment') % 'difference_qty')
        lines = self.search([('sample_id', '=', self.env.context.get('default_sample_id'))])
