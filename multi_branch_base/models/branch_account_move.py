...
class AccountMove(models.Model):
    ...
    def _search_default_journal(self, journal_types):
        ...
        if len(self.env.user.branch_ids) == 1:
            ...
            if not journal:
                if self.partner_id.parent_id.state_sale == '1':
                    if self.branch_id.id == 1:
                        journal = self.env['account.journal'].search([('id','=',59)])
                    if self.branch_id.id == 2:
                        journal = self.env['account.journal'].search([('id','=',63)])
                    if self.branch_id.id == 3:
                        journal = self.env['account.journal'].search([('id','=',67)])
                elif self.partner_id.parent_id.state_sale  == '2':
                    if self.branch_id.id == 1:
                        journal = self.env['account.journal'].search([('id','=',60)])
                    if self.branch_id.id == 2:
                        journal = self.env['account.journal'].search([('id','=',64)])
                    if self.branch_id.id == 3:
                        journal = self.env['account.journal'].search([('id','=',68)])
                elif self.partner_id.parent_id.state_sale  == '3':
                    if self.branch_id.id == 1:
                        journal = self.env['account.journal'].search([('id','=',61)])
                    if self.branch_id.id == 2:
                        journal = self.env['account.journal'].search([('id','=',65)])
                    if self.branch_id.id == 3:
                        journal = self.env['account.journal'].search([('id','=',69)])
                elif self.partner_id.parent_id.state_sale  == '4':
                    if self.branch_id.id == 1:
                        journal = self.env['account.journal'].search([('id','=',62)])
                    if self.branch_id.id == 2:
                        journal = self.env['account.journal'].search([('id','=',66)])
                    if self.branch_id.id == 3:
                        journal = self.env['account.journal'].search([('id','=',70)])
                else:
                    journal = self.env['account.journal'].search(domain, limit=1)
    ...
    branch_id = fields.Many2one('res.branch', string='Branch', store=True,
                                ...
                                domain=_get_branch_domain,required=False)
